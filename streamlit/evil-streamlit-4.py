...
st.markdown(
    "Allocate ≥1,250 t/day across candidate landfill sites, rank by feasibility, and analyze multi-phase horizons."
)
...
# Determine min horizon from selected
selected_sites = [s for s,q in alloc.items() if q>0]
if selected_sites:
    min_horizon = int(ndf.loc[ndf['Site'].isin(selected_sites), 'Service_Horizon_(yr)'].min())
else:
...
if total >= 1250 and sum_d == horizon:
    ...
    for idx, dur in enumerate(durations, 1):
        ...
        tbl = pd.DataFrame({
            ...
            f'Phase{idx}_Rev': rev,
            f'P{idx}_Rev_per_yr': per_yr
        })
        st.table(tbl.round(2))
        st.write(f"**Totals** Cap={cap.sum():,.0f} t·yr, Rev=${rev.sum():,.2f}, Rev/yr avg=${per_yr.sum():,.2f}")
    # Maps
    map_site = st.selectbox("Map view site", df_sel['Site'])
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("Hydro-Risk Map")
        st.info("⚙️ st.image('assets/hydro_risk.png')")
    with c2:
        st.subheader("EJ Index Map")
        st.info("⚙️ st.image('assets/ej_index.png')")
else:
    if total < 1250:
